...
import (
	...
	"github.com/modernprogram/groupcache/v2/testpb"
...
func testSetup() {
	...
	stringGroup = NewGroupWithWorkspace(Options{
		...
		Getter: GetterFunc(func(_ context.Context, key string, dest Sink, info *Info) error {
			if key == fromChan {
				key = <-stringc
			}
			cacheFills.Add(1)
...
func TestPeers(t *testing.T) {
	...
	localHits := 0
	getter := func(_ context.Context, key string, dest Sink, info *Info) error {
		localHits++
...
func TestNoDedup(t *testing.T) {
	...
	g := newGroup(DefaultWorkspace, "testgroup", purgeExpired, 1024,
		...
		GetterFunc(func(_ context.Context,
			key string, dest Sink, info *Info) error {
...
func TestHotUsage(t *testing.T) {
	for i, data := range hotUsageTestTable {
		name := fmt.Sprintf("%d of %d: %s",
			i+1, len(hotUsageTestTable), data.name)
		result := isHotUsageExcessive(data.mainBytes, data.hotBytes,
			data.mainWeight, data.hotWeight)
		if result != data.expected {
			t.Errorf("%s: expected=%t got=%t",
				name, data.expected, result)
		}
	}
}
